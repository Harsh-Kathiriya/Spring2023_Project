...
public class ManagerController {
  ...
  /**
   * @author Sebastian Shirk
   *         {@summary given a provider record creates a provider report for the
   *         given
   *         provider}
   * 
  ...
  ProviderReport requestSingleProviderReport(ProviderRecord currentProvider) {
    ...
    ProviderReport providerReport = new ProviderReport(currentProvider.getName(), currentProvider.getNumber(),
        ...
        currentProvider.getZipCode(), totalConsults, totalFee, dateOfService, dateReceived, memberNames, memberNumbers,
        serviceCodes, serviceFees);
  ...
  MemberReport requestSingleMemberReport(MemberRecord currentMember) {
    ...
    MemberReport memberReport = new MemberReport(currentMember.getName(), currentMember.getNumber(),
        currentMember.getAddress(), currentMember.getCity(), currentMember.getState(), currentMember.getZipCode(),
        dateOfService, providerNames, serviceNames);
    return memberReport;
  }

  /**
   * @author Sebastian Shirk
   *         {@summary creates a member report for every member that participates
   *         in a
   *         service that week}
   */
  ...
  /**
   * @author Sebastian Shirk
   *         {@summary Creates a summary report with every provider, their number
   *         of
   *         constuls and fee and also total number of providers consultations and
   *         fees}
   * 
   * @return
   */
