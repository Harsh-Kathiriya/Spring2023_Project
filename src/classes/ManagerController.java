 ...
 *         {@summary The controller method for the manager. Creates new
 *         provider, member, and summary reports}
 * 
 **/
...
public class ManagerController {
    ...
    public static ProviderRecord getProviderRecordFromFile(String providerID) throws IOException {
        ...
        List<String> lines = Files.readAllLines(Paths.get("./data/Provider_Record"));
    ...
    public static MemberRecord getMemberRecordFromFile(String memberID) throws IOException {
        ...
        List<String> lines = Files.readAllLines(Paths.get("./data/Member_Record"));
    ...
    /**
     * {@summary given a provider record creates a provider report for the given
     * provider}
     * 
     * @param currentProvider
     * @return
     */
    public ProviderReport requestSingleProviderReport(ProviderRecord currentProvider) {
        ...
        ProviderReport providerReport = new ProviderReport(currentProvider.getName(), currentProvider.getNumber(),
                currentProvider.getAddress(), currentProvider.getCity(), currentProvider.getState(),
                currentProvider.getZipCode(), totalConsults, totalFee, dateOfService, dateReceived, memberNames,
                memberNumbers, serviceCodes, serviceFees);
     ...
     */
    public ProviderReport requestProviderReport(String providerId) {
     ...
     * @return
     */
    public MemberReport requestSingleMemberReport(MemberRecord currentMember) {
        ...
        MemberReport memberReport = new MemberReport(currentMember.getName(), currentMember.getNumber(),
                currentMember.getAddress(), currentMember.getCity(), currentMember.getState(),
                currentMember.getZipCode(), dateOfService, providerNames, serviceNames);
        return memberReport;
    }

    /**
     * {@summary creates a member report for every member that participates in a
     * service that week}
     */
    public MemberReport requestMemberReport(String memberId) {
        // create a hash map... the key is the name, the value is an object of all the
        // stuff
        try {
    ...
    /**
     * {@summary Creates a summary report with every provider, their number of
     * constuls and fee and also total number of providers consultations and fees}
     * 
     * @return
     */
